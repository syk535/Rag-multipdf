...
def load_documents(pdf_paths: List[str]) -> List[Document]:
    ...
    for p in pdf_paths:
        ...
        for d in pages:
            ...
            # The loader usually sets 'page', keep it; if missing, best-effort None.
            d.metadata = meta
        docs.extend(pages)
    return docs
...
def split_documents(documents: List[Document], chunk_size=1000, chunk_overlap=100) -> List[Document]:
    ...
    We also attach the *full page text* into metadata so we can later expand boundaries.
    """
    ...
    for page_doc in documents:
        ...
        for d in parts:
            ...
            # 'start_index' is provided by splitter when add_start_index=True
            # Keep the full page text for boundary refine
...
def _expand_span_around(
    ...
    if start_idx is None or end_idx is None:
        # If we can't trust indexes, simply return a middle window of the page
        L = len(full_text)
        s = max(0, (L // 2) - expand_chars)
        e = min(L, s + (end_idx - start_idx if end_idx and start_idx else 600) + 2 * expand_chars)
        return s, e
...
def refine_top_k_chunks(
    ...
    """
    Retrieve top-K by vector store, then refine each chunk by expanding +/- expand_chars
    around its [start, end) span within the *page* full text. Re-score by cosine similarity
    against the query embedding. Return the refined Documents (for LLM context) and an
    analysis string that logs before/after scores and span changes.

    We compute our own cosine scores (query vs. text) to get comparable "before vs after"
    numbers. (FAISS distance is L2, not directly comparable to cosine.)
    """
    # 1) Get query embedding once
    q_vec = embeddings.embed_query(query)

    # 2) Initial retrieval with scores from FAISS
    #    We'll ignore the FAISS numeric score and compute our own cosine to display.
    retrieved: List[Tuple[Document, float]] = vs.similarity_search_with_score(query, k=top_k)

    # 3) For each chunk, re-compute cosine before, perform boundary expand, then cosine after
    refined_docs: List[Document] = []
    analysis_lines: List[str] = []
    analysis_lines.append(f"Top-{top_k} retrieval with boundary refine (±{expand_chars} chars):\n")
    ...
    for rank, (doc, _faiss_score) in enumerate(retrieved, start=1):
        ...
        # LangChain's RecursiveCharacterTextSplitter does not expose chunk length in metadata,
        # so we infer end via start_index + len(text) if start_index is known.
        end_index = (start_index + len(text)) if isinstance(start_index, int) else None
        full_page_text = meta.get("full_page_text", "")

        # Score before (cosine)
        chunk_vec = embeddings.embed_query(text)  # using embed_query for single string is fine
        score_before = _cosine(q_vec, chunk_vec)

        # Expand boundaries over the page text
        if full_page_text and isinstance(start_index, int) and isinstance(end_index, int):
            ...
            # If missing indexes/page text, fall back to just using original text
            refined_text = text
        ...
        # Build a refined doc (clone metadata)
        new_meta = dict(meta)
        new_meta["refined"] = True
        new_meta["refine_expand_chars"] = expand_chars
        new_meta["orig_start_index"] = start_index
        new_meta["orig_end_index"] = end_index
        new_meta["refined_start_index"] = refined_start
        new_meta["refined_end_index"] = refined_end
        new_meta["score_before"] = round(score_before, 6)
        new_meta["score_after"] = round(score_after, 6)
        new_meta["score_delta"] = round(delta, 6)
        ...
        analysis_lines.append(
            ...
            f"(Δ={delta:+.6f})  "
            f"span: { _fmt_span(start_index, end_index) } -> { _fmt_span(refined_start, refined_end) }"
        )

    analysis_lines.append("\nNote: scores shown are cosine similarities (higher is better).")
    analysis = "\n".join(analysis_lines)
...
class ChunkEditor(tk.Toplevel):
    ...
    def _save_edits(self):
        ...
        # If user edited the text length, we cannot keep accurate original end index;
        # we still keep the original 'start_index' and 'full_page_text' for best-effort refine.
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.topk_entry = ttk.Spinbox(bar, from_=1, to=20, textvariable=self.topk_var, width=4)
        ...
        self.expand_entry = ttk.Spinbox(bar, from_=0, to=1000, textvariable=self.expand_var, width=5)
        self.expand_entry.pack(side="left")
        ...
        try:
            ...
            if self.enable_refine.get():
                ...
                for i, d in enumerate(raw_docs, start=1):
                    m = d.metadata or {}
                    names.append(f"#{i} [{Path(m.get('source','doc')).name} p.{(m.get('page')+1) if isinstance(m.get('page'),int) else '?'}]")
