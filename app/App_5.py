...
- Stage 2: At query time, retrieve top-K, then refine each chunk via a sliding-window
           search around its original span to maximize cosine similarity with the query.
...
def _pin_tcl_tk_for_pyinstaller():
    """When frozen by PyInstaller, let Tcl/Tk know where the bundled libraries live."""
...
def pick_pdfs(parent) -> List[str]:
    """Open a file dialog to select one or more PDFs."""
    return list(
...
def load_documents(pdf_paths: List[str]) -> List[Document]:
    ...
    Warn if no extractable text was detected (likely scanned PDFs).
    """
    docs = []
    for p in pdf_paths:
...
def split_documents(documents: List[Document], chunk_size=1000, chunk_overlap=100) -> List[Document]:
    ...
    Critical: add_start_index=True to remember start offset within the page text.
...
def build_vectorstore(chunks: List[Document], embeddings: OpenAIEmbeddings) -> FAISS:
    """Build a FAISS VectorStore from chunks."""
    return FAISS.from_documents(chunks, embeddings)


def format_docs_for_context(docs: List[Document], max_chars=1200) -> str:
    """
    Compose concise context blocks: [source p.X] + excerpt.
    Deduplicate near-duplicates by (source, page, hash(prefix)).
    """
    rows, seen = [], set()
    for d in docs:
        meta = d.metadata or {}
...
def build_chain(vectorstore: FAISS):
    """
    Create the base RAG generation chain (without using retriever inside).
    We handle retrieval (and refine) outside to access scores.
    """
    llm = ChatOpenAI(model="gpt-4o-mini", temperature=0, timeout=60, max_retries=1)

    SYSTEM = ""  # Intentionally empty; you can add guidance if needed.
...
def _clamp(a: int, lo: int, hi: int) -> int:
    """Clamp integer a into [lo, hi]."""
    return max(lo, min(hi, a))


def _best_window_by_sliding(
    full_text: str,
    center: int,
    base_len: int,
    expand_steps: List[int],
    slide_steps: int,
    slide_step_chars: int,
    score_fn,  # callable: text -> float
):
    """
    Generate multiple candidate windows around the original center, with various lengths
    and left/right shifts, then return the best-scoring (start, end, text, score).

    - base_len: original chunk length (len(original_text))
    - expand_steps: e.g. [0, E, 2E] where E is the 'Refine ±chars' value from UI
    - slide_steps: how many offsets to try on each side (total = 2*slide_steps + 1)
    - slide_step_chars: shift size for each step (in characters)
    """
    L = len(full_text)
    best = (-1.0, 0, 0, "")  # (score, s, e, text)

    # For each target length, slide left/right around center
    for extra in expand_steps:
        win_len = max(50, base_len + 2 * extra)  # keep a reasonable minimum
        half = win_len // 2

        for k in range(-slide_steps, slide_steps + 1):
            offset = k * slide_step_chars
            s = _clamp(center - half + offset, 0, L)
            e = _clamp(s + win_len, 0, L)
            if e <= s:
                continue
            cand = full_text[s:e]
            sc = score_fn(cand)
...
def refine_top_k_chunks(
    ...
    """
    Retrieve top-K, then for each chunk do a sliding-window search around its original span.
    We try multiple window lengths (base_len + 2*expand) and multiple left/right shifts,
    score each candidate via cosine similarity with the query embedding, and pick the best.
    """
    from pathlib import Path
    ...
    # FAISS retrieval (doc, score) where doc.page_content is the original chunk text
    retrieved: List[Tuple[Document, float]] = vs.similarity_search_with_score(query, k=top_k)

    refined_docs: List[Document] = []
    lines: List[str] = []
    lines.append(f"Top-{top_k} retrieval with boundary refine (sliding windows, step={expand_chars} chars):\n")
    ...
    for rank, (doc, _faiss_score) in enumerate(retrieved, start=1):
        ...
        # score_before (on the ORIGINAL chunk)
        chunk_vec = embeddings.embed_query(text)
        score_before = _cosine(q_vec, chunk_vec)

        # If we don't have enough info to refine, keep the original
        if not (full_page_text and isinstance(start_index, int) and isinstance(end_index, int)):
            refined_text = text
            ...
            skipped_reason = None
            base_len = max(10, end_index - start_index)  # original length
            center = (start_index + end_index) // 2

            # Candidate window lengths: base, base±(2*expand), base±(4*expand) (clipped later by text bounds)
            # We keep them non-decreasing and within reasonable ranges.
            expand_steps = sorted(set([
                0,
                expand_chars // 2,
                expand_chars,
                2 * expand_chars
            ...
            # Slide offsets: from -N to +N steps, each of size slide_step_chars
            slide_steps = 2  # total 5 positions: -2, -1, 0, +1, +2
            slide_step_chars = max(20, expand_chars // 2)

            # Define a local scoring function using the cached query vector
            def _score_fn(snippet: str) -> float:
                vec = embeddings.embed_query(snippet)
                return _cosine(q_vec, vec)

            best_score, s_new, e_new, refined_text = _best_window_by_sliding(
                full_text=full_page_text,
                center=center,
                base_len=base_len,
                expand_steps=expand_steps,
                slide_steps=slide_steps,
                slide_step_chars=slide_step_chars,
                score_fn=_score_fn,
            )
        ...
        # Build refined doc with enriched meta
        new_meta = dict(meta)
        new_meta.update({
            "refined": True,
            "refine_mode": "sliding_window",
            "refine_step_chars": expand_chars,
        ...
        block = [
            ...
            f"span: { _fmt_span(start_index, end_index) } -> { _fmt_span(refined_start, refined_end) }{('' if not skipped_reason else skipped_reason)}",
            "",
            "— ORIGINAL —",
...
class ChunkEditor(tk.Toplevel):
    ...
    def __init__(self, parent, chunks, on_done):
        ...
        # Bottom buttons (full width; visible on small screens)
        ...
        # Force topmost to avoid being hidden under the main window (common in EXE)
        self.lift()
    ...
    def _save_edits(self):
        ...
        # Note: if user edited the text length, the original end index loses exactness,
